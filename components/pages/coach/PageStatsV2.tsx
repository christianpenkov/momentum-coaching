...
function TabOverview({ ig, yt, stripe, shortio, msgs, calls, period }: { ig: IGStats | null; yt: YTStats | null; stripe: StripeStats | null; shortio: ShortioStats | null; msgs: IGMessages | null; calls: CallRecord[]; period: Period }) {
  ...
  const callsByPlatform = (platform: 'IG' | 'YT') => {
    const src = platform === 'IG' ? 'instagram' : 'youtube';
    const platformCalls = calls.filter(c => (c.source || '').toLowerCase().startsWith(src));
    const honores = platformCalls.filter(c => c.status === 'active' && new Date(c.scheduled_at) < now && !c.no_show);
    const noShowCount = platformCalls.filter(c => c.no_show).length;
    const closedCount = platformCalls.filter(c => c.deal_closed).length;
    const noShowPct = platformCalls.length > 0 ? Math.round((noShowCount / platformCalls.length) * 100) : null;
    const closedPct = honores.length > 0 ? Math.round((closedCount / honores.length) * 100) : null;
    return { noShowCount, noShowPct, closedCount, closedPct };
  };
  ...
  const allContent: ContentItem[] = [
    ...(ig?.posts.map(p => {
      const { noShowCount, noShowPct, closedCount, closedPct } = callsByPlatform('IG');
      const callsBooked = Math.round((p.reach || 0) * 0.001);
      const revTotal = callsBooked * Math.round(mrr / Math.max(dealsCloses, 1));
      ...
      return {
        ...
        noShowCount, noShowPct, closedCount, closedPct,
        callsBooked,
    ...
    ...(yt?.videos.map(v => {
      const { noShowCount, noShowPct, closedCount, closedPct } = callsByPlatform('YT');
      const callsBooked = Math.round(v.views30d * 0.0002);
      const revTotal = callsBooked * Math.round(mrr / Math.max(dealsCloses, 1));
          ...
          <div key={i} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '18px 20px 14px' }}>
            ...
            <ResponsiveContainer width="100%" height={140}>
              <ReAreaChart data={item.data} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                ...
                <XAxis dataKey="date" tick={{ fontSize: 10, fill: 'var(--muted)' }} axisLine={false} tickLine={false} interval={period === 7 ? 0 : 4} />
          ...
          <thead>
            <tr>
              {((): string[] => {
                ...
                if (contentSort === 'calls') return ['', 'Contenu', 'Plateforme', 'Calls bookés', 'No-show', 'Closé'];
...
function TabInstagram({ ig, period }: { ig: IGStats | null; period: Period }) {
  ...
  return (
    <div className="stack">
      <StatGrid>
        <Stat label="Abonnés" value={fmt(ig.followers)} />
        <Stat label="Reach 30j" value={fmt(ig.reach30d)} />
        <Stat label="Vues 30j" value={fmt(ig.views30d)} />
        <Stat label="Comptes engagés 30j" value={fmt(ig.accountsEngaged30d)} />
        <Stat label="Interactions 30j" value={fmt(ig.totalInteractions30d)} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
        <Card title="Top pays">
          {(ig.demographics?.country || []).length > 0
            ? (ig.demographics.country || []).map((d, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                <div style={{ fontSize: 12, color: 'var(--muted)', width: 40 }}>{d.label}</div>
                <div style={{ flex: 1, height: 6, background: 'var(--border)', borderRadius: 3 }}>
                  <div style={{ height: 6, width: `${pct(d.value, ig.demographics.country[0]?.value || 1)}%`, background: ACCENT, borderRadius: 3 }} />
                </div>
                <div style={{ fontSize: 12, fontWeight: 600, width: 30, textAlign: 'right' }}>{d.value}</div>
              </div>
            ))
            : <Empty msg="Données insuffisantes" />}
        </Card>
        <Card title="Top villes">
          {(ig.demographics?.city || []).length > 0
            ? (ig.demographics.city || []).map((d, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                <div style={{ fontSize: 12, color: 'var(--muted)', width: 80, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{d.label}</div>
                <div style={{ flex: 1, height: 6, background: 'var(--border)', borderRadius: 3 }}>
                  <div style={{ height: 6, width: `${pct(d.value, ig.demographics.city[0]?.value || 1)}%`, background: GREEN, borderRadius: 3 }} />
                </div>
                <div style={{ fontSize: 12, fontWeight: 600, width: 30, textAlign: 'right' }}>{d.value}</div>
              </div>
            ))
...
function TabYouTube({ yt, period }: { yt: YTStats | null; period: Period }) {
  ...
  return (
    <div className="stack">
      <StatGrid>
        <Stat label="Abonnés" value={fmt(yt.subscribers)} />
        <Stat label="Vues all-time" value={fmt(yt.totalViews)} />
        <Stat label="Vidéos" value={fmt(yt.videoCount)} />
        <Stat label="Vues 30j" value={fmt(yt.views30d)} />
        <Stat label="Watch time total 30j" value={`${watchTimeH}h`} sub={`${Math.round(watchTimeH * 60 / 1000)}k min au total`} />
        <Stat label="Watch time moyen / vue" value={yt.avgViewDurationSec ? `${Math.floor(yt.avgViewDurationSec / 60)}m${String(yt.avgViewDurationSec % 60).padStart(2, '0')}s` : '—'} sub="durée moyenne par vue" />
        <Stat label="Likes 30j" value={fmt(yt.likes30d)} />
        <Stat label="Commentaires 30j" value={fmt(yt.comments30d)} />
        <Stat label="Partages 30j" value={fmt(yt.shares30d)} />
        <Stat label="Subs gagnés 30j" value={`+${fmt(yt.subsGained30d)}`} color={GREEN} />
        <Stat label="Subs perdus 30j" value={`-${fmt(yt.subsLost30d)}`} color={RED} />
        <Stat label="Subs nets 30j" value={`${yt.netSubs30d >= 0 ? '+' : ''}${fmt(yt.netSubs30d)}`} color={yt.netSubs30d >= 0 ? GREEN : RED} />
        <Stat label="Conv. vue→sub" value={`${conversionRate}%`} sub="subs gagnés / vues" />
      </StatGrid>
...
function TabFunnel({ msgs, calls, stripe, ig, yt, shortio, period }: { msgs: IGMessages | null; calls: CallRecord[]; stripe: StripeStats | null; ig: IGStats | null; yt: YTStats | null; shortio: ShortioStats | null; period: Period }) {
  const [tooltip, setTooltip] = useState<string | null>(null);
  const now = new Date();
  const callsBookes = calls.filter(c => c.status === 'active').length;
  const callsHonores = calls.filter(c => c.status === 'active' && new Date(c.scheduled_at) < now && !c.no_show).length;
  const noShows = calls.filter(c => c.status === 'active' && new Date(c.scheduled_at) < now && c.no_show).length;
  const dealsCloses = calls.filter(c => c.deal_closed).length;
  const mrr = stripe?.mrr || 0;
  const totalReach = (ig?.reach30d || 0) + (yt?.views30d || 0);
  const totalViews = (ig?.views30d || 0) + (yt?.views30d || 0);
  const shortioClicks = shortio?.humanClicks30d || 0;
  const contentCount = (ig?.posts.length || 0) + (yt?.videos.length || 0);
  const subs = (ig?.followers || 0) + (yt?.subscribers || 0);
  const watchTimeH = yt ? yt.watchTime30d / 60 : 0;
  const revPer1kViews = totalViews > 0 ? mrr / (totalViews / 1000) : 0;
  ...
  const revPerSub = subs > 0 ? (mrr / subs) * 1000 : 0;
  const callsPer100hWatch = watchTimeH > 0 ? (callsBookes / watchTimeH) * 100 : 0;
  const revPerCall = callsHonores > 0 ? mrr / callsHonores : 0;
  const noShowRate = callsBookes > 0 ? pct(noShows, callsBookes) : 0;
  const closingRate = callsHonores > 0 ? pct(dealsCloses, callsHonores) : 0;

  const funnelSteps = [
    { label: 'Reach', value: totalReach, sub: 'IG + YT' },
    { label: 'Clicks', value: shortioClicks, sub: 'vers Calendly', rate: totalReach > 0 ? (shortioClicks / totalReach) * 100 : 0 },
    { label: 'Calls', value: callsBookes, sub: 'bookés', rate: shortioClicks > 0 ? (callsBookes / shortioClicks) * 100 : 0 },
    { label: 'Showed', value: callsHonores, sub: 'honorés', rate: callsBookes > 0 ? (callsHonores / callsBookes) * 100 : 0 },
    { label: 'Closed', value: dealsCloses, sub: 'deals', rate: callsHonores > 0 ? (dealsCloses / callsHonores) * 100 : 0 },
    { label: 'MRR', value: mrr, sub: 'revenu récurrent', isMrr: true },
  ];

  const pipelineSteps = [
    { label: 'DMs', value: msgs?.totalThreads30d || 0 },
    { label: 'Leads', value: msgs?.leadCount || 0, rate: msgs?.totalThreads30d ? pct(msgs.leadCount, msgs.totalThreads30d) : 0, rateLabel: 'Lead rate' },
    { label: 'Calls bookés', value: callsBookes, rate: msgs?.leadCount ? pct(callsBookes, msgs.leadCount) : 0, rateLabel: 'Booking rate' },
    { label: 'Calls honorés', value: callsHonores, rate: callsBookes ? pct(callsHonores, callsBookes) : 0, rateLabel: 'Show rate' },
    { label: 'Deals closés', value: dealsCloses, rate: callsHonores ? pct(dealsCloses, callsHonores) : 0, rateLabel: 'Close rate' },
  ];

  const maxPipeline = pipelineSteps[0].value || 1;

  const leads = msgs?.threads.filter(t => t.isLead) || [];

  const efficiencyMetrics = [
    { label: 'Revenue / 1k Views', value: fmtEur(Math.round(revPer1kViews)), tooltip: 'MRR divisé par le total des vues × 1 000' },
    { label: 'Calls / 10k Reach', value: fmt(callsPer10kReach, 2), tooltip: 'Calls bookés divisé par le reach × 10 000' },
    { label: 'Revenue / Content', value: fmtEur(Math.round(revPerContent)), tooltip: 'MRR divisé par le nombre de contenus publiés' },
    { label: 'Revenue / 1k Followers', value: fmtEur(Math.round(revPerSub)), tooltip: 'MRR divisé par le nombre d\'abonnés × 1 000' },
    { label: 'Calls / 100h Watch', value: fmt(callsPer100hWatch, 2), tooltip: 'Calls bookés divisé par le watch time × 100h' },
    { label: 'Revenue / Call', value: fmtEur(Math.round(revPerCall)), tooltip: 'MRR divisé par le nombre de calls honorés' },
  ];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 48 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 0, borderRadius: 16, overflow: 'hidden', border: '1px solid var(--border)' }}>
        {[
          { label: 'Reach', value: totalReach >= 1000 ? `${fmt(totalReach / 1000, 1)}k` : fmt(totalReach), sub: 'IG + YT — 30j' },
          { label: 'Calls', value: fmt(callsBookes), sub: `${callsHonores} honorés` },
          { label: 'Deals', value: fmt(dealsCloses), sub: `${closingRate}% closing` },
          { label: 'MRR', value: fmtEur(mrr), sub: 'revenu récurrent', accent: true },
        ].map((h, i) => (
          <div key={i} style={{
            padding: '32px 28px',
            background: h.accent ? 'var(--accent)' : 'var(--surface)',
            borderLeft: i > 0 ? '1px solid var(--border)' : 'none',
          }}>
            <div style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.08em', color: h.accent ? 'rgba(255,255,255,.7)' : 'var(--muted)', marginBottom: 10 }}>{h.label}</div>
            <div style={{ fontSize: 36, fontWeight: 800, color: h.accent ? '#fff' : 'var(--ink)', lineHeight: 1, marginBottom: 6 }}>{h.value}</div>
            <div style={{ fontSize: 12, color: h.accent ? 'rgba(255,255,255,.6)' : 'var(--muted)' }}>{h.sub}</div>
          </div>
        ))}
      </div>

      {/* ── SECTION 2 — FUNNEL VISUEL ── */}
      <div>
        <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.08em', color: 'var(--muted)', marginBottom: 32 }}>Funnel</div>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0 }}>
          {funnelSteps.map((step, i) => {
            const isLast = i === funnelSteps.length - 1;
            const width = Math.max(30, 100 - i * 13);
            return (
              <div key={i} style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                <div style={{
                  width: `${width}%`,
                  padding: '20px 28px',
                  background: step.isMrr ? 'var(--accent)' : 'var(--surface)',
                  border: `1px solid ${step.isMrr ? 'var(--accent)' : 'var(--border)'}`,
                  borderRadius: 12,
                  display: 'flex',
                  alignItems: 'center',
                  ...
                  <div>
                    <div style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.07em', color: step.isMrr ? 'rgba(255,255,255,.7)' : 'var(--muted)', marginBottom: 4 }}>{step.label}</div>
                    <div style={{ fontSize: 32, fontWeight: 800, color: step.isMrr ? '#fff' : 'var(--ink)', lineHeight: 1 }}>
                      {step.isMrr ? fmtEur(step.value as number) : (step.value as number) >= 1000 ? `${fmt((step.value as number) / 1000, 1)}k` : fmt(step.value as number)}
                    </div>
                  </div>
                  <div style={{ fontSize: 12, color: step.isMrr ? 'rgba(255,255,255,.6)' : 'var(--muted)' }}>{step.sub}</div>
                </div>
                {!isLast && (
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '10px 0', gap: 2 }}>
                    ...
                    {'rate' in step && step.rate !== undefined && (
                      <div style={{ fontSize: 11, fontWeight: 600, color: (step.rate as number) < 2 ? RED : (step.rate as number) < 10 ? AMBER : GREEN }}>
                        {fmt(step.rate as number, 1)}%
                      </div>
                    )}
                  </div>
                )}
      ...
      <div>
        <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.08em', color: 'var(--muted)', marginBottom: 20 }}>Efficiency Metrics</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
          {efficiencyMetrics.map((m, i) => (
            <div key={i}
              style={{ padding: '20px 22px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, cursor: 'default', position: 'relative' }}
              onMouseEnter={() => setTooltip(m.tooltip)}
              onMouseLeave={() => setTooltip(null)}
            >
              <div style={{ fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.07em', color: 'var(--muted)', marginBottom: 8 }}>{m.label}</div>
              <div style={{ fontSize: 28, fontWeight: 800, color: 'var(--ink)', lineHeight: 1 }}>{m.value}</div>
              {tooltip === m.tooltip && (
                <div style={{
                  position: 'absolute', bottom: 'calc(100% + 8px)', left: 0, right: 0,
                  background: 'var(--ink)', color: 'var(--surface)', fontSize: 11, padding: '6px 10px',
                  borderRadius: 6, zIndex: 10, lineHeight: 1.5, pointerEvents: 'none',
                }}>
                  {m.tooltip}
                </div>
              )}
      ...
      <div>
        <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.08em', color: 'var(--muted)', marginBottom: 20 }}>Pipeline</div>
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
          {pipelineSteps.map((step, i) => {
            const barWidth = maxPipeline > 0 ? (step.value / maxPipeline) * 100 : 0;
            return (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '140px 1fr 80px', alignItems: 'center', gap: 16, padding: '16px 24px', borderTop: i > 0 ? '1px solid var(--border-soft)' : 'none' }}>
                <div style={{ fontSize: 12, color: 'var(--muted)', fontWeight: 500 }}>{step.label}</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ flex: 1, height: 6, background: 'var(--surface-2)', borderRadius: 3, overflow: 'hidden' }}>
                    <div style={{ width: `${barWidth}%`, height: '100%', background: 'var(--accent)', borderRadius: 3, transition: 'width .4s ease' }} />
                  </div>
                  <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--ink)', minWidth: 36, textAlign: 'right' }}>{fmt(step.value)}</div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  {'rate' in step && step.rate !== undefined && i > 0 && (
                    <div style={{ fontSize: 12, fontWeight: 600, color: step.rate < 20 ? AMBER : GREEN }}>{fmtPct(step.rate)}</div>
                  )}
                </div>
              </div>
            );
      ...
      {/* ── SECTION 5 — DM INTELLIGENCE ── */}
      <div>
        <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.08em', color: 'var(--muted)', marginBottom: 20 }}>DM Intelligence</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>

          {/* Keywords */}
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '20px 22px' }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--ink)', marginBottom: 16 }}>Top Keywords</div>
            {msgs && Object.keys(msgs.keywordCounts).length > 0 ? (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {Object.entries(msgs.keywordCounts).sort((a, b) => b[1] - a[1]).map(([kw, count], i) => (
                  <div key={i} style={{
                    display: 'flex', alignItems: 'center', gap: 6,
                    background: 'var(--surface-2)', border: '1px solid var(--border)',
                    borderRadius: 20, padding: '5px 12px',
                  }}>
                    <span style={{ fontSize: 12, fontWeight: 600 }}>{kw}</span>
                    <span style={{ fontSize: 10, color: 'var(--muted)', background: 'var(--border)', borderRadius: 10, padding: '1px 6px' }}>{count}</span>
                  </div>
                ))}
              </div>
            ) : <div style={{ fontSize: 12, color: 'var(--faint)' }}>Aucun keyword détecté</div>}
          </div>

          {/* Conversation status */}
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '20px 22px' }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--ink)', marginBottom: 16 }}>Conversation Status</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12 }}>
              {[
                { label: 'Répondus', value: msgs?.repliedThreads || 0, color: GREEN },
                { label: 'Sans réponse', value: (msgs?.totalThreads30d || 0) - (msgs?.repliedThreads || 0), color: AMBER },
                { label: 'Leads', value: msgs?.leadCount || 0, color: 'var(--accent)' },
                { label: 'Taux réponse', value: fmtPct(msgs?.responseRate || 0), color: 'var(--ink)' },
              ].map((s, i) => (
                <div key={i} style={{ padding: '12px 14px', background: 'var(--surface-2)', borderRadius: 8 }}>
                  <div style={{ fontSize: 10, color: 'var(--muted)', marginBottom: 4 }}>{s.label}</div>
                  <div style={{ fontSize: 20, fontWeight: 700, color: s.color }}>{typeof s.value === 'number' ? fmt(s.value) : s.value}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {leads.length > 0 && (
          <div style={{ marginTop: 16, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
            <div style={{ padding: '16px 22px', borderBottom: '1px solid var(--border-soft)' }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--ink)' }}>Recent Leads</div>
            </div>
            {leads.slice(0, 5).map((t, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 22px', borderTop: i > 0 ? '1px solid var(--border-soft)' : 'none' }}>
                <div style={{ width: 34, height: 34, borderRadius: '50%', background: 'var(--surface-2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, flexShrink: 0 }}>
                  {t.participant?.[0]?.toUpperCase() || '?'}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 600 }}>@{t.participant}</div>
                  <div style={{ fontSize: 11, color: 'var(--muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.preview || '(media)'}</div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                  <span style={{ fontSize: 10, background: AMBER + '20', color: AMBER, borderRadius: 4, padding: '3px 8px', fontWeight: 700 }}>LEAD</span>
                  <div style={{ fontSize: 10, color: 'var(--muted)' }}>{new Date(t.updatedAt).toLocaleDateString('fr-FR')}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ── SECTION 6 — CALLS TABLE ── */}
      <div>
        <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.08em', color: 'var(--muted)', marginBottom: 20 }}>Calls</div>
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
          <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', display: 'flex', gap: 20 }}>
            {[
              { label: 'Total calls', value: fmt(calls.length), color: 'var(--ink)' },
              { label: 'Honorés', value: fmt(calls.filter(c => c.status === 'active' && new Date(c.scheduled_at) < now && !c.no_show).length), color: GREEN },
              { label: 'No-show', value: fmt(calls.filter(c => c.no_show).length), color: RED },
              { label: 'Closés', value: fmt(calls.filter(c => c.deal_closed).length), color: GREEN },
            ].map((s, i) => (
              <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {calls.slice(0, 20).map((c, i) => {
                ...
                const statusLabel = isCanceled ? 'Annulé' : c.no_show ? 'No-show' : isPast ? 'Honoré' : 'À venir';
                const statusColor = isCanceled ? RED : c.no_show ? RED : isPast ? GREEN : 'var(--muted)';
                const [srcPlatform, srcMedium] = (c.source || '').split('_');
                return (
                  <tr key={i} style={{ borderTop: '1px solid var(--border-soft)' }}>
                    ...
                    <td style={{ padding: '12px 14px' }}>
                      {c.source ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                          <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--ink)', textTransform: 'capitalize' }}>{srcPlatform}</span>
...
const MOCK_IG: IGStats = {
  ...
  followsUnfollows30d: 380, profileLinksTaps30d: 1240, websiteClicks30d: 890,
...
const MOCK_MSGS: IGMessages = {
  totalThreads30d: 148, repliedThreads: 124, responseRate: 83.8, leadCount: 42,
...
const MOCK_CALLS: CallRecord[] = [
  { id: 'c1', scheduled_at: new Date(Date.now() + 2*86400000).toISOString(), status: 'active', invitee_name: 'Thomas Renard', invitee_email: 'thomas@email.com', duration: 60, source: 'Instagram' },
  { id: 'c2', scheduled_at: new Date(Date.now() + 4*86400000).toISOString(), status: 'active', invitee_name: 'Sarah Martin', invitee_email: 'sarah@email.com', duration: 60, source: 'YouTube' },
  { id: 'c3', scheduled_at: new Date(Date.now() - 3*86400000).toISOString(), status: 'active', invitee_name: 'Lucas Bernard', invitee_email: 'lucas@email.com', duration: 60, source: 'Instagram' },
  { id: 'c4', scheduled_at: new Date(Date.now() - 7*86400000).toISOString(), status: 'active', invitee_name: 'Emma Dupont', invitee_email: 'emma@email.com', duration: 60, source: 'Short.io' },
  { id: 'c5', scheduled_at: new Date(Date.now() - 10*86400000).toISOString(), status: 'active', no_show: true, invitee_name: 'Mehdi Amrani', invitee_email: 'mehdi@email.com', duration: 60 },
  { id: 'c6', scheduled_at: new Date(Date.now() - 14*86400000).toISOString(), status: 'active', deal_closed: true, revenue: 1200, invitee_name: 'Julie Fontaine', invitee_email: 'julie@email.com', duration: 60, source: 'Instagram' },
  { id: 'c7', scheduled_at: new Date(Date.now() - 18*86400000).toISOString(), status: 'active', deal_closed: true, revenue: 1800, invitee_name: 'Kevin Laurent', invitee_email: 'kevin@email.com', duration: 60, source: 'YouTube' },
  { id: 'c8', scheduled_at: new Date(Date.now() - 22*86400000).toISOString(), status: 'canceled', invitee_name: 'Camille Rey', invitee_email: 'camille@email.com', duration: 60 },
];

const MOCK_SHORTIO: ShortioStats = {
  domain: 'qnl.link', totalLinks: 14, clicks30d: 8420, humanClicks30d: 7180,
  clicksChange: 18.4, clicksPerLink30d: 601.4,
  chartData: makeDays(30, 280, 140).map(d => ({ date: d.date, clicks: d.value })),
  topCountries: [{ label: 'France', value: 5840 }, { label: 'Belgique', value: 980 }, { label: 'Suisse', value: 620 }, { label: 'Canada', value: 480 }, { label: 'Maroc', value: 280 }],
  topReferrers: [{ label: 'instagram.com', value: 4200 }, { label: 'Direct', value: 2100 }, { label: 't.co', value: 840 }, { label: 'youtube.com', value: 680 }, { label: 'linktr.ee', value: 420 }],
  topBrowsers: [{ label: 'Chrome', value: 4820 }, { label: 'Safari', value: 2140 }, { label: 'Firefox', value: 680 }, { label: 'Edge', value: 380 }],
  topOs: [{ label: 'Android', value: 3840 }, { label: 'iOS', value: 2940 }, { label: 'Windows', value: 980 }, { label: 'macOS', value: 440 }],
  topSocial: [{ label: 'Instagram', value: 4200 }, { label: 'Twitter/X', value: 840 }, { label: 'Facebook', value: 420 }, { label: 'LinkedIn', value: 280 }],
  topCities: [{ label: 'Paris', value: 1840 }, { label: 'Lyon', value: 680 }, { label: 'Marseille', value: 520 }, { label: 'Bruxelles', value: 440 }, { label: 'Genève', value: 320 }],
  links: [
    { id: 1, path: 'coaching', shortUrl: 'qnl.link/coaching', originalUrl: 'https://quennel.com/coaching', title: 'Page Coaching', createdAt: new Date(Date.now() - 60*86400000).toISOString(), clicks30d: 3240, humanClicks30d: 2980, clicksChange: 22.1, chartData: makeDays(30, 108, 50).map(d => ({ date: d.date, clicks: d.value })), countries: [{ label: 'France', value: 2100 }], referrers: [{ label: 'instagram.com', value: 1840 }], browsers: [{ label: 'Chrome', value: 1800 }], os: [{ label: 'Android', value: 1600 }], social: [{ label: 'Instagram', value: 1840 }], cities: [{ label: 'Paris', value: 840 }], utmMedium: [], utmSource: [] },
    { id: 2, path: 'appel', shortUrl: 'qnl.link/appel', originalUrl: 'https://calendly.com/quennel/discovery', title: 'Réserver un appel', createdAt: new Date(Date.now() - 45*86400000).toISOString(), clicks30d: 2180, humanClicks30d: 2040, clicksChange: 31.4, chartData: makeDays(30, 72, 36).map(d => ({ date: d.date, clicks: d.value })), countries: [{ label: 'France', value: 1400 }], referrers: [{ label: 'instagram.com', value: 1200 }], browsers: [{ label: 'Safari', value: 980 }], os: [{ label: 'iOS', value: 1100 }], social: [{ label: 'Instagram', value: 1200 }], cities: [{ label: 'Paris', value: 560 }], utmMedium: [], utmSource: [] },
    { id: 3, path: 'yt', shortUrl: 'qnl.link/yt', originalUrl: 'https://youtube.com/@quennel', title: 'Chaîne YouTube', createdAt: new Date(Date.now() - 30*86400000).toISOString(), clicks30d: 1480, humanClicks30d: 1320, clicksChange: 8.2, chartData: makeDays(30, 49, 24).map(d => ({ date: d.date, clicks: d.value })), countries: [{ label: 'France', value: 980 }], referrers: [{ label: 'instagram.com', value: 720 }], browsers: [{ label: 'Chrome', value: 840 }], os: [{ label: 'Android', value: 740 }], social: [{ label: 'Instagram', value: 720 }], cities: [{ label: 'Paris', value: 380 }], utmMedium: [], utmSource: [] },
    { id: 4, path: 'temoignages', shortUrl: 'qnl.link/temoignages', originalUrl: 'https://quennel.com/resultats', title: 'Témoignages clients', createdAt: new Date(Date.now() - 20*86400000).toISOString(), clicks30d: 840, humanClicks30d: 760, clicksChange: -4.8, chartData: makeDays(30, 28, 16).map(d => ({ date: d.date, clicks: d.value })), countries: [{ label: 'France', value: 540 }], referrers: [{ label: 'Direct', value: 380 }], browsers: [{ label: 'Chrome', value: 460 }], os: [{ label: 'Android', value: 400 }], social: [{ label: 'Instagram', value: 280 }], cities: [{ label: 'Paris', value: 210 }], utmMedium: [], utmSource: [] },
  ],
};

export default function PageStatsV2() {
  ...
  useEffect(() => {
    setIg(MOCK_IG); setYt(MOCK_YT); setStripe(MOCK_STRIPE);
    setMsgs(MOCK_MSGS); setShortio(MOCK_SHORTIO); setCalls(MOCK_CALLS);
    setLoading(false);
  }, []);

  const TABS = ['Vue générale', 'Instagram', 'YouTube', 'Funnel & Calls', 'Revenus', 'Short.io'];
    ...
    <div className="page-content">
      <div className="page-header" style={{ marginBottom: 20, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          ...
          {useMocks && (
            <div style={{ fontSize: 11, background: AMBER + '20', color: AMBER, borderRadius: 6, padding: '5px 12px', fontWeight: 600, border: `1px solid ${AMBER}40` }}>
      ...
      {loading ? <Loading /> : (
        <>
          {tab === 0 && <TabOverview ig={ig} yt={yt} stripe={stripe} shortio={shortio} msgs={msgs} calls={calls} period={period} />}
          {tab === 1 && <TabInstagram ig={ig} period={period} />}
          {tab === 2 && <TabYouTube yt={yt} period={period} />}
          {tab === 3 && <TabFunnel msgs={msgs} calls={calls} stripe={stripe} ig={ig} yt={yt} shortio={shortio} period={period} />}
          {tab === 4 && <TabRevenues stripe={stripe} period={period} />}
          {tab === 5 && <TabShortio shortio={shortio} period={period} />}
        </>
      )}
    </div>
  );
