...
interface Props {
  ...
  onClose: () => void;
  profileId: string;
  initialDomains: ShortioDomainOption[];
...
// "Changer de domaine" depuis Réglages. Écrit le choix dans integrations.metadata via
...
export default function ShortioDomainPicker({ open, onClose, profileId, initialDomains, currentDomainId, onSelected }: Props) {
  const [domains, setDomains] = useState<ShortioDomainOption[]>(initialDomains);
  ...
  if (result) {
    return (
      <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
        <div style={{ background: 'var(--surface)', borderRadius: 12, padding: 20, width: 420, maxWidth: '90vw' }}>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={onClose}>
      <div style={{ background: 'var(--surface)', borderRadius: 12, padding: 20, width: 420, maxWidth: '90vw' }} onClick={e => e.stopPropagation()}>
