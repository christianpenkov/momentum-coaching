-- ⚠️ FICHIER RECONSTITUÉ le 2026-09-07 depuis `supabase_migrations.schema_migrations.statements`
-- (méthode d'AGENTS.md « Le SQL exact d'une migration appliquée est RÉCUPÉRABLE »).
-- La migration avait été appliquée le 2026-09-06 à 17:25 UTC sans que son fichier soit
-- commité, puis la fonctionnalité a été ABANDONNÉE le même jour par
-- 20260906201000_retirer_mot_cle_permanent.sql (décision de Chris). Sans ce fichier,
-- `migrations_sante` la signalait « appliquée sans fichier » — une alerte permanente,
-- c'est-à-dire un e-mail quotidien pour rien. L'histoire complète reste lisible :
-- ajout ici, retrait dans la migration suivante, effet net nul sur le schéma.
-- Contenu ci-dessous : copie exacte des instructions appliquées.
--
-- ────────────────────────────────────────────────────────────────────────────
